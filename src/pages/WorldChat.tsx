...
export default function WorldChat() {
  ...
  async function submitEdit(msgId: string) {
    ...
    } else { setMessages(um); const aid = crypto.randomUUID(); const am: Message = { id: aid, role: 'assistant', content: '', thinking: '', timestamp: Date.now() }; setMessages(prev => [...prev, am]); setLoading(true); setError(''); isStreamingRef.current = true; setThinkingOpen(p => ({ ...p, [aid]: true })); try { let ctx: Message[] = []; if (worldId) { const w = getWorld(worldId); if (w) { let st = ''; if (w.setting.worldview) st += `【世界观】\n${w.setting.worldview}\n\n`; if (w.setting.myCharacter.name) st += `【角色】${w.setting.myCharacter.name}\n\n`; if (w.setting.specialRules) st += `【规则】\n${w.setting.specialRules}\n\n`; if (w.setting.writingPreferences) st += `【写作偏好】\n${w.setting.writingPreferences}\n\n`; if (st) ctx.push({ id: 'ws', role: 'system', content: st.trim(), timestamp: 0 }) } } if (!worldId) { const ms = localStorage.getItem('kadath-main-summary'); if (ms) ctx.push({ id: 'ms', role: 'system', content: `以下是之前对话的摘要：\n\n${ms}`, timestamp: 0 }) } await sendMessageStream([...ctx, ...um], c => { setMessages(p => p.map(m => m.id === aid ? { ...m, thinking: (m.thinking||'')+c } : m)) }, c => { setThinkingOpen(p => ({ ...p, [aid]: false })); setMessages(p => p.map(m => m.id === aid ? { ...m, content: m.content+c } : m)) }, u => { setMessages(p => p.map(m => m.id === aid ? { ...m, usage: u } : m)) }, abortRef.current?.signal) } catch (e) { if (e instanceof Error && e.name !== 'AbortError') setError(e.message) } finally { setLoading(false); isStreamingRef.current = false; setMessages(p => { localStorage.setItem(getChatKey(worldId), JSON.stringify(p)); return p }) } }
  }
